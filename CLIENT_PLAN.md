...
### 1.1 Theme tokens (CSS variables, toggled by `[data-theme]`)
...
### 1.2 Foundations
- Typography: one modern sans (Inter/Poppins), clear type scale, generous spacing, 8px spacing grid.
- Cards with soft radius (12–16px) + subtle shadow (light) / subtle border (dark). Smooth 150–200ms transitions.
- **Theme toggle** in the top bar; preference saved to `localStorage` (`az-theme`) and applied before paint to avoid flash. Respects `prefers-color-scheme` on first visit.
- Fully **responsive** (mobile-first): collapsible side panel → hamburger drawer on small screens.
- Accessible: focus rings, aria labels, keyboard-navigable wizard & tables.
...
/AzCuts
└── /client
    ├── package.json                 # Deps + scripts ("start", "build"). Deps: react-router-dom, axios, @tanstack/react-query, socket.io-client, react-hook-form, recharts, html2canvas, dayjs, react-hot-toast, lucide-react (icons).
    ...
    └── /src
        ...
        │
        └── /styles
            ├── theme.css            # CSS variables for light/dark tokens (§1.1).
            ├── globals.css          # Base/reset, typography, utilities.
            └── (or Tailwind config) # If Tailwind chosen: tailwind.config with theme tokens + darkMode:'class'.
```

> **Styling choice:** Tailwind CSS (with `darkMode:'class'`) is recommended for speed + consistent tokens; plain CSS Modules + `theme.css` variables is the fallback. Either way the token names in §1.1 stay the same.
...
- **Phase 0 — Skeleton & design system:** CRA/Vite init, install deps, Router, providers, `theme.css` tokens + **light/dark toggle**, `/ui` base components, DashboardShell/Sidebar/Topbar, Axios instance. **DoD:** app runs, theme toggle works, empty routes render.
